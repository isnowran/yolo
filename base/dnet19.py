...
class darknet19(nn.Module):
    def __init__(self, anchors, num_classes):
        ...
        self.stage3_conv1 = nn.Sequential(nn.Conv2d(256 + 1024, 1024, 3, 1, 1, bias=False), nn.BatchNorm2d(1024),
                                          nn.LeakyReLU(0.1, inplace=True))
        self.stage3_conv2 = nn.Conv2d(1024, len(self.anchors) * (self.rsize), 1, 1, 0, bias=False)

    def forward(self, imgx):
        output = self.stage1_conv1(imgx)
        ...
        return libnet.refine(output, self.rsize)

if __name__ == "__main__":
    import conf
    net = darknet19(conf.anchors, 20)
    isize = 416
    img = torch.rand(4, 3, isize, isize)
    r = net(img)
    print r.shape
